...
def test_ncc_and_mosse_grids_are_independent_objects() -> None:
    ...
    be fed into the other method's validator and raise -- this pins the split. ``mosse``'s grid is
    now a strict superset of ``ncc``'s on this branch (the base scales x retain_frac x nms_iou
    block is byte-identical, plus ``mosse``'s additive cardinal-bank x mirror block from the
    floor-plan investigation, which has no ``ncc`` equivalent here -- ``ncc``'s own floor-plan grid
    entries were added independently by the sibling quick task).
    """
    ncc_grid, mosse_grid = _TUNING_GRIDS["ncc"], _TUNING_GRIDS["mosse"]
    assert ncc_grid is not mosse_grid
    assert len(mosse_grid) > len(ncc_grid)
    # The shared base block (ncc's whole grid) is byte-identical and present verbatim in mosse's.
    base = [dict(o) for o in ncc_grid]
    assert [dict(o) for o in mosse_grid[: len(ncc_grid)]] == base
    # ...and no individual override dict in the shared prefix is aliased either.
    assert all(a is not b for a, b in zip(mosse_grid[: len(ncc_grid)], ncc_grid, strict=True))
