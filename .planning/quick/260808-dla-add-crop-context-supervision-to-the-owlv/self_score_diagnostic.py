"""Local, CPU-only self_score comparison across all four fine-tuned OWLv2 checkpoints.

Per D-dla-07: the original 260805-hg1 diagnostic ran ad hoc on a since-destroyed vast.ai
instance against an exemplar whose exact coordinates were never persisted, so it cannot be
re-run bit-for-bit. This script picks a FRESH, deterministic exemplar instead -- the first
door-class ground-truth box, by annotation list order, in the first file_name-sorted training
image that has one -- and measures ``self_score`` (the exact quantity
``owlv2_oneshot.search()``'s self-similarity calibration computes, see ``owlv2_oneshot.py``
step 6) for all four checkpoints against that SAME fixed exemplar. The four numbers are
honestly comparable to each other; they are NOT bit-identical to the previously-reported ad
hoc numbers (a different exemplar), which is disclosed in the report rather than implied away.

Run entirely locally, no GPU:
    pixi run python .planning/quick/260808-dla-add-crop-context-supervision-to-the-owlv/\
