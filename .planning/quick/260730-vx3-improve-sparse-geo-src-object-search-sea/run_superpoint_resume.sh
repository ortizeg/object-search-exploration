#!/usr/bin/env bash
# Resume driver for quick task 260730-vx3 hypothesis 2. Runs only the conditions whose
# result JSON is missing, so an interrupted session is resumed rather than redone.
# Identical grids to run_superpoint.sh; logs land in this directory (NOT /tmp).
set -u
...
run() {  # run <label> <dataset> <grid>
  ...
  if [ -s "$DIR/measurements/${label}.json" ]; then
    echo "=== SKIP  $label (result already on disk) ==="
    return
  fi
