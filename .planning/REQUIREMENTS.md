...
- [ ] **INFRA-01**: Pixi environment, Python 3.12, all commands runnable via `pixi run`
- [ ] **INFRA-02**: src-layout package with `py.typed`; `pyproject.toml` is the single
      source of truth for package metadata
- [ ] **INFRA-03**: Ruff (line-length 100) and MyPy strict both run clean over the package
- [ ] **INFRA-04**: Pre-commit hooks installed before the first commit
- [ ] **INFRA-05**: Loguru only for logging — no `print()`, no stdlib `logging`, enforced by
      lint
- [ ] **INFRA-06**: Pytest with a ≥80% coverage gate that fails the build below the floor
