...
_NCC_GRID: tuple[dict[str, object], ...] = _correlation_grid()
...
def run_domain_tuning(
    ...
    * ``exemplar_counts`` given (e.g. ``(1, 3)``) -- each count is tuned independently and the
    ...
    Args:
        ...
        grids: Optional per-method override of :data:`_TUNING_GRIDS` (``None`` -- the default --
            falls back to the committed grid for every method). Lets a research script sweep a
            method-specific variant (e.g. a floor-plan rotation-bank sweep) without shelling out to
            the full CLI, which always tunes every registered method with its committed grid.
